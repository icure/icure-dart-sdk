...
class KeywordDto {
  ...
  @override
  String toString() =>
      'KeywordDto[id=$id, rev=$rev, created=$created, modified=$modified, author=$author, responsible=$responsible, medicalLocationId=$medicalLocationId, tags=$tags, codes=$codes, endOfLife=$endOfLife, deletionDate=$deletionDate, value=$value, subWords=$subWords, userId=$userId]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "KeywordDto[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "KeywordDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<KeywordDto>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = KeywordDto.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, KeywordDto> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = KeywordDto.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
