    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "KeywordSubwordDto[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "KeywordSubwordDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
      ...
      return KeywordSubwordDto(
        ...
        subWords: KeywordSubwordDto.listFromJson(json[r'subWords']) ?? const [],
      );
    }
    return null;
  }
  ...
  static List<KeywordSubwordDto>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = KeywordSubwordDto.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, KeywordSubwordDto> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = KeywordSubwordDto.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
