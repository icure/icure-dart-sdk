//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.0

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

part of openapi.api;

class ByteArrayDto {
  /// Returns a new [ByteArrayDto] instance.
  ByteArrayDto({
    this.data = const [],
  });

  List<String> data;

  @override
  bool operator ==(Object other) => identical(this, other) || other is ByteArrayDto &&
     other.data == data;

  @override
  int get hashCode =>
  // ignore: unnecessary_parenthesis
    (data == null ? 0 : data.hashCode);

  @override
  String toString() => 'ByteArrayDto[data=$data]';

  Map<String, dynamic> toJson() {
    final json = <String, dynamic>{};
    if (data != null) {
      json[r'data'] = data;
    }
    return json;
  }

  /// Returns a new [ByteArrayDto] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ByteArrayDto fromJson(dynamic value) {
    if (value is Map) {
      final json = value.cast<String, dynamic>();
      return ByteArrayDto(
        data: json[r'data'] is List
          ? (json[r'data'] as List).cast<String>()
          : null,
      );
    }
    return null;
  }

  static List<ByteArrayDto> listFromJson(dynamic json, {bool emptyIsNull, bool growable,}) =>
    json is List && json.isNotEmpty
      ? json.map(ByteArrayDto.fromJson).toList(growable: true == growable)
      : true == emptyIsNull ? null : <ByteArrayDto>[];

  static Map<String, ByteArrayDto> mapFromJson(dynamic json) {
    final map = <String, ByteArrayDto>{};
    if (json is Map && json.isNotEmpty) {
      json
        .cast<String, dynamic>()
        .forEach((key, dynamic value) => map[key] = ByteArrayDto.fromJson(value));
    }
    return map;
  }

  // maps a json object with a list of ByteArrayDto-objects as value to a dart map
  static Map<String, List<ByteArrayDto>> mapListFromJson(dynamic json, {bool emptyIsNull, bool growable,}) {
    final map = <String, List<ByteArrayDto>>{};
    if (json is Map && json.isNotEmpty) {
      json
        .cast<String, dynamic>()
        .forEach((key, dynamic value) {
          map[key] = ByteArrayDto.listFromJson(
            value,
            emptyIsNull: emptyIsNull,
            growable: growable,
          );
        });
    }
    return map;
  }
}

