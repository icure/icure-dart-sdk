    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "NoGenericPrescriptionReasonDto[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "NoGenericPrescriptionReasonDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<NoGenericPrescriptionReasonDto>? listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = NoGenericPrescriptionReasonDto.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, NoGenericPrescriptionReasonDto> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = NoGenericPrescriptionReasonDto.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
