...
class TimeSeriesDto {
  ...
  static List<TimeSeriesDto>? listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<TimeSeriesDto>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<TimeSeriesDto>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = TimeSeriesDto.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
