    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "PharmaceuticalFormDto[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "PharmaceuticalFormDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PharmaceuticalFormDto>? listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = PharmaceuticalFormDto.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, PharmaceuticalFormDto> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = PharmaceuticalFormDto.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
