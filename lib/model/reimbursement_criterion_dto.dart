    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ReimbursementCriterionDto[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ReimbursementCriterionDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ReimbursementCriterionDto>? listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ReimbursementCriterionDto.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ReimbursementCriterionDto> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = ReimbursementCriterionDto.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
