    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "TimeTableDto[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "TimeTableDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<TimeTableDto>? listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = TimeTableDto.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, TimeTableDto> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = TimeTableDto.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
