...
class FilterChainDevice {
  ...
  static List<FilterChainDevice> listFromJson(dynamic json, {bool emptyIsNull, bool growable,}) =>
    json is List && json.isNotEmpty
      ? json.map(FilterChainDevice.fromJson).toList(growable: true == growable)
      : true == emptyIsNull ? null : <FilterChainDevice>[];
  ...
  static Map<String, FilterChainDevice> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      json
        .cast<String, dynamic>()
        .forEach((key, dynamic value) => map[key] = FilterChainDevice.fromJson(value));
    }
    return map;
  }

  // maps a json object with a list of FilterChainDevice-objects as value to a dart map
  static Map<String, List<FilterChainDevice>> mapListFromJson(dynamic json, {bool emptyIsNull, bool growable,}) {
    final map = <String, List<FilterChainDevice>>{};
    if (json is Map && json.isNotEmpty) {
      json
        .cast<String, dynamic>()
        .forEach((key, dynamic value) {
          map[key] = FilterChainDevice.listFromJson(
            value,
