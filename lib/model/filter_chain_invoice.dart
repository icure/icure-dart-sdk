...
class FilterChainInvoice {
  ...
  bool operator ==(Object other) =>
      identical(this, other) || other is FilterChainInvoice &&
          other.filter == filter &&
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "FilterChainInvoice[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "FilterChainInvoice[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<FilterChainInvoice>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <FilterChainInvoice>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = FilterChainInvoice.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, FilterChainInvoice> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = FilterChainInvoice.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<FilterChainInvoice>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<FilterChainInvoice>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = FilterChainInvoice.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
