...
class DecryptedContactDto {
  ...
  static List<DecryptedContactDto>? listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<DecryptedContactDto>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<DecryptedContactDto>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = DecryptedContactDto.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
