...
class PaginatedListNmpDto {
  ...
  bool operator ==(Object other) =>
      identical(this, other) || other is PaginatedListNmpDto &&
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "PaginatedListNmpDto[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "PaginatedListNmpDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PaginatedListNmpDto>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <PaginatedListNmpDto>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = PaginatedListNmpDto.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, PaginatedListNmpDto> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = PaginatedListNmpDto.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<PaginatedListNmpDto>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<PaginatedListNmpDto>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PaginatedListNmpDto.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
