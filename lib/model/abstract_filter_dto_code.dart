...
class AbstractFilterDtoCode {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "AbstractFilterDtoCode[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "AbstractFilterDtoCode[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<AbstractFilterDtoCode>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <AbstractFilterDtoCode>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = AbstractFilterDtoCode.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, AbstractFilterDtoCode> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = AbstractFilterDtoCode.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<AbstractFilterDtoCode>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<AbstractFilterDtoCode>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = AbstractFilterDtoCode.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
