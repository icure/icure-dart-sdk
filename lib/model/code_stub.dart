    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CodeStub[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CodeStub[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<CodeStub>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = CodeStub.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, CodeStub> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = CodeStub.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
