...
class PaginatedListPatientDto {
  ...
  bool operator ==(Object other) =>
      identical(this, other) || other is PaginatedListPatientDto &&
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "PaginatedListPatientDto[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "PaginatedListPatientDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PaginatedListPatientDto>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <PaginatedListPatientDto>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = PaginatedListPatientDto.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, PaginatedListPatientDto> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = PaginatedListPatientDto.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<PaginatedListPatientDto>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<PaginatedListPatientDto>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PaginatedListPatientDto.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
