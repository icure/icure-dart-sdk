...
extension ContactCryptoConfig on CryptoConfig<DecryptedContactDto, ContactDto> {
  ...
  Future<ContactDto> encryptContact(String myId, Set<String> delegations, DecryptedContactDto contact) async {
    ...
    if (!eks.entries.any((s) => s.value.isNotEmpty)) {
      ...
      final secretForDelegates = await Future.wait((<String>{...delegations, myId}).map((String d) async => Tuple2(
          d, await this.crypto.encryptAESKeyForHcp(myId, d, contact.id, secret.toHexString())
      )));
      eks = {...eks, ...Map.fromEntries(secretForDelegates.map((t) => MapEntry(t.item1, <DelegationDto>{DelegationDto(
        owner: myId, delegatedTo: t.item1, key: t.item2
      )})))};
    } else {
...
extension ContactApiCrypto on ContactApi {
  Future<DecryptedContactDto?> createContact(UserDto user, DecryptedContactDto contact,
      CryptoConfig<DecryptedContactDto, ContactDto> config) async {
    var newContact = await this.createContact(
        await config.encryptContact(
  ...
  Future<DecryptedContactDto?> createContactWithPatient(UserDto user, DecryptedPatientDto patient, DecryptedContactDto contact, CryptoConfig<DecryptedContactDto, ContactDto> config) async {
    var delegations = <String>{...(user.autoDelegations["all"] ?? {}), ...(user.autoDelegations["medicalInformation"] ?? {})};
    var encContact = await config.encryptContact(
            user.healthcarePartyId!,
            delegations,
            contact
        );
    final secret = (await config.crypto.decryptEncryptionKeys(user.healthcarePartyId!, patient.delegations))
        .firstOrNull();
    if (secret == null) {
      throw FormatException("Cannot get delegation key for ${patient.id} and hcp ${user.healthcarePartyId}");
    }

    final secretForDelegates = await Future.wait((<String>{...delegations, user.healthcarePartyId!}).map((String d) async => Tuple2(
        d, await config.crypto.encryptValueForHcp(user.healthcarePartyId!, d, contact.id, patient.id)
    )));
    encContact.cryptedForeignKeys = {...encContact.cryptedForeignKeys, ...Map.fromEntries(secretForDelegates.map((t) => MapEntry(t.item1, <DelegationDto>{DelegationDto(
        owner: user.healthcarePartyId!, delegatedTo: t.item1, key: t.item2
    )})))};
  ...
  Future<DecryptedContactDto?> modifyContact(UserDto user, DecryptedContactDto contact,
      CryptoConfig<DecryptedContactDto, ContactDto> config) async {
    var newContact = await this.modifyContact(
        await config.encryptContact(
            user.healthcarePartyId!,
            <String>{...(user.autoDelegations["all"] ?? {}), ...(user.autoDelegations["medicalInformation"] ?? {})},
            contact
...
Future<List<DecryptedContactDto>?> createContacts(UserDto user, List<DecryptedContactDto> contacts, CryptoConfig<DecryptedContactDto, ContactDto> config) async {
  ...
  Future<List<DecryptedContactDto>?> modifyContacts(UserDto user, List<DecryptedContactDto> contacts, CryptoConfig<DecryptedContactDto, ContactDto> config) async {
    var modifiedContacts = await this.modifyContacts(await Future.wait(contacts.map((contact) => config.encryptContact(
        user.healthcarePartyId!,
        <String>{...(user.autoDelegations["all"] ?? {}), ...(user.autoDelegations["medicalInformation"] ?? {})},
        contact
    ))));
    return modifiedContacts == null ? null :
