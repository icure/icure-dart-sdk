...
extension AccessLogCryptoConfig on CryptoConfig<DecryptedAccessLogDto, AccessLogDto> {
  Future<AccessLogDto> encryptAccessLog(String myId, Set<String> delegations, DecryptedAccessLogDto accessLogDto) async {
    ...
    Future<DecryptedAccessLogDto> getEncryptedAccessLog(DecryptedAccessLogDto accessLog) async {
      if (accessLogDto.encryptionKeys.entries.any((element) => element.value.isNotEmpty)) {
        return accessLogDto;
      } else {
    ...
    final DecryptedAccessLogDto decryptedAccessLogDto = await getEncryptedAccessLog(accessLogDto);
